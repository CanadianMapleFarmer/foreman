...
- [x] **Step 1: Install into Kustom and commit the baseline**
...
- [x] **Step 2: Restart the MCP connection and run doctor**
...
- [x] **Step 3: Smoke with the cheapest reviewer (cost under $0.01)**
...
- [x] **Step 4: End-to-end coding task**
...
End-to-end task `health-route-01` on Kustom, driven through the real `foreman mcp` server with the MCP client SDK against live OpenCode.

| Stage | Model | Wall time | Reported cost |
|---|---|---|---|
| Coder turn (route file, `bun install`, route tree regen, Biome, build) | openrouter/z-ai/glm-5.3-flash | 72 s | $0.0049 |
| Gate (`cd apps/web && bun install --frozen-lockfile && bun run check && bun run build`) | foreman | 2 s | 0 |
| Review (final, clean JSON: 0 blocking, 0 warnings) | openrouter/deepseek/deepseek-v4-flash-0731 | 10 s | $0.0006 |
| Merge (`--no-ff` into main, worktree removed) | foreman | 3 s | 0 |
| Whole task including two failed review attempts and one follow-up | | | $0.0121 |

OpenRouter credit: $20.000 at start of day, $19.967 after research probes, smoke test and this task.

Smoke test `smoke-01` (planner, qwen3.7-flash, "reply PONG"): 7 s, $0.0004.

Defects found and fixed during the run:
1. `worker_wait` blocked longer than the MCP client's 60 s request timeout. Default is now 45 s with a 300 s max; callers poll on `status: running`.
2. Orphaned server after client death: the server now exits when stdin closes.
3. Cost under-reported: the ACP prompt result carries only the last step's tokens. OpenCode's `usage_update.cost` is now preferred.
4. Reviewer's shell commands (`bun install`, `bun run build`, `cat x 2>/dev/null`) were rejected by the read policy, and DeepSeek ends its turn silently after a rejected call. Read policy now whitelists inspection commands and package scripts (edits, redirects, `sed -i`, `rm`, `mv`, `mkdir` stay blocked); the review prompt states the gate result; an empty reply is retried once with a JSON-only nudge.
5. Follow-ups after a foreman restart failed with "session not found". Sessions are now reloaded with `session/load` and the model re-pinned (loading resets it to the default).
6. Fresh worktrees lack `node_modules` and gitignored env files: `copyIntoWorktree` config plus an install step in the acceptance command.

Observed: OpenCode asked the client for `fs/write_text_file` despite `fs` capabilities being declared false; the client answers method-not-supported and OpenCode falls back to its own file tools without harm.

Open: reload `.mcp.json` in the Claude Code session (restart) so the `foreman` tools appear natively; until then the driver scripts in the session scratchpad exercise the same server.
